			...
			<tr>
				...
				<c:if test="${not empty list}">
					<c:forEach var="n" items="${list }" varStatus="status">
						<tr>
							<td>${status.count}</td>
							<td><a class="link" href="${pageContext.request.contextPath }/notice/notice_contentView?notice_num=${n.notice_num}">${n.notice_title}</a></td>
							<td>${n.notice_date }</td>
							<td>${n.notice_hits }</td>
						</tr>
					</c:forEach>
				...
				<c:if test="${sessionScope.user_type == 2 }">
					<tr>
						...
						<input class="writing_btn" type="button" id="Writing" onclick="location.href = '${pageContext.request.contextPath }/notice/notice_writing'" value="공지사항 작성"></td>
					</tr>
				</c:if>
		</table>
	</div>
